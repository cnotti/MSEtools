...
for (j in 1:1) {
  for (i in 1:n_cases) {
    ...
    names(loc_s) = c("longitude", "latitude")
    ns = nrow(loc_s)
    ...
    areaOmega = geosphere::areaPolygon(domain)

    loc_f = coordinates(spsample(domain,
                                 floor(areaOmega/areadredge),
                                 type = "regular"))
    nf = nrow(loc_f)
    ...
    # run model
    cat(paste("running case where domain area =", round(areaOmega/1e6, 2), "km sq\n"))
    om_out[[i]] = om(data_om = data_om, seed = seed)
...
if (FALSE) {
  ...
  for (c in 1:data_om[["nc"]]) {
    for (t in 1:data_om[["nt"]]) {
      ...
      B_cst[c,data_om[["splus_s"]]+1,t] = om_rep[["B_cst"]][c,,t]/areaOmega*ns
    }
  }
